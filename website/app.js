    ...
    style.textContent = `
        ...
        .winner-arrival {
            ...
            box-shadow:
                ...
                rgba(0,0,0,0.35);
            animation:
                winnerPop 420ms ease-out forwards;
        }

        .winner-arrival small {
            display: block;
            margin-bottom: 5px;
            font-size: 0.78rem;
            font-weight: 800;
        ...
        .winner-arrival strong {
            display: block;
            font-size:
                clamp(1.45rem, 4vw, 2.35rem);
            line-height: 1.05;
        }

        .winner-arrival .winner-trophy {
            display: block;
            margin-bottom: 8px;
            font-size: 3rem;
        }

        .winner-arrival.leaving {
            animation:
                winnerFade 500ms ease-out
                forwards;
        }
...
function fireworkExplosion() {
    document
        .querySelectorAll('.winner-fireworks')
...
function showWinnerPopup() {
    document
        .querySelectorAll('.winner-arrival')
        .forEach(el => el.remove());
...
function update() {
    ...
    benchmarkMatrix.innerHTML =
        matrixMarkup(modelData.cm);


    benchmarkCurveTitle.textContent =
        `${modelData.name}${winnerText} · training and validation accuracy and loss`;


    benchmarkCurveImage.src =
        `${RAW}/benchmark/figures/${modelData.name}_learning_curves.png`;


    benchmarkCurveImage.alt =
        `${modelData.name} training and validation accuracy and loss curves`;
    ...
    architecture.innerHTML = `
        ...
        <a
            class="return-link"
            href="theory.html#${architectureSlugs[modelData.name]}"
        >
            Open the ${modelData.name}
    ...
    bChart.data.datasets[0].data =
        keys.map(
            key => modelData[key]
        );


    bChart.options.plugins.title.text =
        modelData.winner
    ...
    /*
       ...
       This makes the Chart.js canvas
       redraw immediately, so an enlarged
       snapshot cannot accidentally show
       the previous model.
    */
    bChart.update('none');
}
...
function updateFine() {
    ...
    fineCount.textContent =
        `${fineView + 1} of ${fineViews.length}`;
    ...
    fChart.options.plugins.legend.display =
        fineView === 2;
    ...
    if (fineView === 0) {
        ...
        finePanel.innerHTML = `
            ...
            <p class="matrix-balance">
                ...
                The same 1,028 evaluation
                images were redistributed
    ...
    fChart.options.scales.y.min =
        fineView === 3
            ? -7
            : 84;


    fChart.options.scales.y.max =
        fineView === 3
            ? 7
            : 100;


    fChart.options.scales.y.title.text =
        fineView === 3
            ? 'Change (percentage points)'
            : 'Performance (%)';


    fChart.options.scales.y.ticks.callback =
        fineView === 3
            ? value =>
                (value > 0
                    ? '+'
    ...
    const frozenUrl =
        `${RAW}/benchmark/figures/InceptionResNetV2_learning_curves.png`;


    const tunedUrl =
        `${RAW}/fine_tuning/figures/FineTuned_InceptionResNetV2_learning_curves.png`;
    ...
    fChart.options.plugins.title.text =
        `InceptionResNetV2 · ${view.title}`;
...
function setupExpandableVisuals() {
    ...
    function visualName(source) {
        ...
        if (
            ...
            return `${modelData.name}${winnerText}`;
        }
        ...
        if (
            ...
            return `${modelData.name}${winnerText} · Confusion matrix`;
        }
        ...
        if (
            ...
            return `${modelData.name}${winnerText} · Training and validation curves`;
        }
        ...
        if (
            ...
            return `InceptionResNetV2 · ${fineViews[fineView].title}`;
        }
        ...
        if (
            ...
            return `InceptionResNetV2 · ${fineViews[fineView].title} learning curves`;
        }
    ...
    function createExpandedCopy(
        ...
        /*
           Canvas cannot be safely cloned.

           Convert the current rendered
           Chart.js canvas into an image.
        */
        if (canvas) {
    ...
    async function open(source) {
        ...
        /*
           Every enlarged benchmark
           sequence starts at VGG16.
        */
        if (
            benchmarkVisuals.has(
                source.id
        ...
        /*
           Wait until Chart.js has been
           painted before taking the
           enlarged canvas snapshot.
        */
