...
function matrixMarkup(cm){return `<div class="matrix-axis top">Predicted class</div><div class="matrix-axis side">True class</div><div class="matrix-labels columns"><span>Restricted</span><span>Unrestricted</span></div><div class="matrix-labels rows"><span>Restricted</span><span>Unrestricted</span></div><div class="matrix-cells"><div class="cm-tp"><small>True positive</small><b>${cm.tp}</b></div><div class="cm-fn"><small>False negative</small><b>${cm.fn}</b></div><div class="cm-fp"><small>False positive</small><b>${cm.fp}</b></div><div class="cm-tn"><small>True negative</small><b>${cm.tn}</b></div></div>`}
function matrixDifferenceMarkup(){return `<div class="matrix-axis top">Predicted class</div><div class="matrix-axis side">True class</div><div class="matrix-labels columns"><span>Restricted</span><span>Unrestricted</span></div><div class="matrix-labels rows"><span>Restricted</span><span>Unrestricted</span></div><div class="matrix-cells matrix-differences"><div class="delta-bad"><small>True positive</small><b>−2</b></div><div class="delta-bad"><small>False negative</small><b>+2</b></div><div class="delta-good"><small>False positive</small><b>−14</b></div><div class="delta-good"><small>True negative</small><b>+14</b></div></div>`}
function update(){const m=models[i],s=story(i),keys=['Accuracy','Precision','Recall','F1'];model.textContent=m.name;year.textContent=m.year;benchmarkMetricsModel.textContent=m.name;benchmarkMatrixModel.textContent=m.name;headline.textContent=s[0];commentary.textContent=s[1];prev.disabled=i===0;next.disabled=i===5;next.textContent='Next →';tiles.innerHTML=keys.map(k=>`<div class="tile" style="--c:${colors[k]}"><small>${k}</small><b>${m[k].toFixed(1)}%</b></div>`).join('');benchmarkMatrix.innerHTML=matrixMarkup(m.cm);benchmarkCurveTitle.textContent=`${m.name} · training and validation accuracy and loss`;benchmarkCurveImage.src=`${RAW}/benchmark/figures/${m.name}_learning_curves.png`;benchmarkCurveImage.alt=`${m.name} training and validation accuracy and loss curves`;architecture.innerHTML=`<div><small>HIGH-LEVEL ARCHITECTURE · INPUT ${m.input}</small><h3>${m.name}: what happens inside?</h3><p>${m.idea}</p></div><div class="architecture-route">${m.route.map((x,j)=>`${j?'<i>→</i>':''}<b>${x}</b>`).join('')}</div><p class="architecture-why"><strong>Why it matters:</strong> ${m.why}</p><a class="return-link" href="theory.html#${architectureSlugs[m.name]}">Open the ${m.name} architecture explanation →</a>`;if(i===0)deltas.innerHTML='<div class="delta"><span>Comparison</span><b>Starting point</b></div>';else{const p=models[i-1];deltas.innerHTML=keys.map(k=>{const d=+(m[k]-p[k]).toFixed(1),cl=Math.abs(d)<.05?'':d>0?'up':'down',ar=Math.abs(d)<.05?'→':d>0?'↑':'↓';return `<div class="delta ${cl}"><span>${k} vs ${p.name}</span><b>${ar} ${d>0?'+':''}${d.toFixed(1)} pts</b></div>`}).join('')}bChart.data.datasets[0].data=keys.map(k=>m[k]);bChart.options.plugins.title.text=`${m.name} · test metrics`;bChart.update()}
...
function updateFine(){
  ...
  if(fineView===0){
    ...
    finePanel.innerHTML=`<div class="visual-identity"><small>CONFUSION MATRIX · BEFORE FINE-TUNING</small><strong>InceptionResNetV2</strong></div><p class="eyebrow">Frozen CNN base</p><div class="compact-matrix">${matrixMarkup(frozenMatrix)}</div><a class="matrix-help" href="metrics.html#confusion-matrix">How to read this matrix →</a>`;
  }else if(fineView===1){
    fChart.data.datasets=[performanceDataset('After fine-tuning',fineTuned,metricKeys.map(k=>colors[k]))];
    finePanel.innerHTML=`<div class="visual-identity"><small>CONFUSION MATRIX · AFTER FINE-TUNING</small><strong>InceptionResNetV2</strong></div><p class="eyebrow">Final 20 layers trainable</p><div class="compact-matrix">${matrixMarkup(fineTunedMatrix)}</div><a class="matrix-help" href="metrics.html#confusion-matrix">How to read this matrix →</a>`;
  }else if(fineView===2){
    ...
    finePanel.innerHTML=`<div class="visual-identity"><small>CONFUSION MATRIX · FINE-TUNED MINUS FROZEN</small><strong>InceptionResNetV2</strong></div><div class="compact-matrix difference-matrix">${matrixDifferenceMarkup()}</div><p class="matrix-balance"><strong>Zero-sum check:</strong> +14 − 14 + 2 − 2 = 0. The same 1,028 test images were redistributed among the four cells.</p>`;
  }
...
function setupExpandableVisuals(){
  ...
  overlay.hidden=true;
  overlay.innerHTML='<div class="visual-overlay-toolbar"><button class="visual-model-nav visual-prev" type="button">← Previous</button><strong id="visualOverlayTitle">Expanded visual</strong><button class="visual-model-nav visual-next" type="button">Next →</button><span>Press Esc to close</span><button class="visual-close" type="button" aria-label="Close expanded visual">×</button></div><div class="visual-overlay-content"></div>';
