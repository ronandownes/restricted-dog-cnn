...
The complete report, poster, presentation and slide sources are maintained in [`latex/`](latex/README.md). They can be compiled locally in VS Code, while the GitHub Actions build is started manually only when refreshed public PDFs are required. The project does not depend on Overleaf.
